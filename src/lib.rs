...
pub fn parse_json_points(path: &str) -> Vec<PtValue> {
    let mut file = File::open(path).unwrap();
    let mut raw_json = String::new();
    file.read_to_string(&mut raw_json).unwrap();
    let decoded: ValuesJson = json::decode(&raw_json).unwrap();
...
pub fn save_json_points(path: &str, obs_points: Vec<Vec<PtValue>>){
    let mut res = Vec::new();
...
pub fn smooth(reso_lat: u32, reso_lon: u32, bbox: Bbox, obs_points: &mut [PtValue], configuration: Config) -> Vec<Vec<PtValue>> {
